...
GetRequiredRtvVotes(activePlayers)
{
    ...
    new votes = floatround(activePlayers * 0.6, floatround_ceil);
    if (votes < 2)
        votes = 2;
    return votes;
}
...
public CmdNominate(id)
{
    ...
    new menu = menu_create("\yESKIDOSTLAR BHOP\d | Nominate a Map", "NominateMenuHandler");
    new count = 0;
    ...
    menu_setprop(menu, MPROP_EXITNAME, BMOD_MENU_HOME);
...
public NominateMenuHandler(id, menu, item)
{
    ...
    {
        menu_destroy(menu);
        if (LibraryExists("bhop_timer", LibType_Library)) bmod_open_main_menu(id);
        return PLUGIN_HANDLED;
    }
...
ShowVoteMenu(id)
{
    new menu = menu_create("\yESKIDOSTLAR BHOP\d | RTV Next Map Vote", "VoteMenuHandler");
...
stock bool:IsCountedRtvPlayer(id)
{
...
ManagerChat(id, const fmt[], any:...)
{
    ...
    for (new player = 1; player <= MAX_PLAYERS; player++)
    {
        ...
        {
            message_begin(MSG_ONE_UNRELIABLE, g_msgTeamInfo, _, player);
            write_byte(player);
            write_string("TERRORIST");
            message_end();
